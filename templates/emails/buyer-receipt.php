<?php
/**
 * Buyer receipt email template (gold-grade, table-based, inline styles).
 *
 * Sent to the customer after a successful order. Header and footer are
 * supplied by Abstract_Email::send() — this template emits the order
 * body content only.
 *
...
<p class="nx-text" style="<?php echo esc_attr( $nx_p_style ); ?>">
    ...
    echo wp_kses_post(
        sprintf(
            ...
            __( 'Your order <strong>#%1$s</strong> was placed on <strong>%2$s</strong> and is being processed.', 'tejcart' ),

<?php
/**
 * Low stock alert email template (gold-grade, inline styles).
 *
 * Sent to the store admin when a product's stock falls to or below the
 * configured low-stock threshold. Header and footer are supplied by
 * Abstract_Email::send().
 *
...
<p class="nx-text" style="<?php echo esc_attr( $nx_p_style ); ?>">
    ...
    echo wp_kses_post(
        sprintf(
            /* translators: 1: product name, 2: SKU, 3: remaining stock */
            __( 'Product <strong>%1$s</strong> (SKU: %2$s) is low on stock. Only <strong>%3$d</strong> remaining.', 'tejcart' ),
